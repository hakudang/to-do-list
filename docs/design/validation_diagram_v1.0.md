...
```mermaid
flowchart TD
    A[User nhập nội dung task] --> B{Validation}
    B -->|Hợp lệ| C[Thêm/Cập nhật task]
    B -->|Không hợp lệ| D[Hiển thị lỗi, không thay đổi dữ liệu]
    C --> E[Lưu vào LocalStorage]
    E --> F[Hiển thị task mới/cập nhật]
    D --> G[Giữ nguyên dữ liệu hiện tại]
```